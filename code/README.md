...
| [`scout_queue.py`](data/scout_queue.py) | One durable, rate-limited queue for every automated prose scout run. |
...
| [`news_audit.py`](data/news_audit.py) | Read and explain immutable news-pulse decision records. |
...
| [`newswatch.py`](in-season/newswatch.py) | Twenty-minute injury and opportunity watcher. |
...
| [`construction.py`](in-season/construction.py) | Roster construction: one check that runs after anything reshapes the roster. |
| [`quality.py`](in-season/quality.py) | The Player Quality Engine (PQI) for waiver valuation and bidding. |
| [`transactions.py`](in-season/transactions.py) | Every roster change, one row each: what happened, when, and who asked for it. |
...
| [`value_history.py`](in-season/value_history.py) | Historical snapshots and like-for-like deltas for expected player value. |
...
| [`faab_field.py`](in-season/faab_field.py) | Opponent-by-opponent waiver demand and bid distributions. |
...
| [`ui_player_card.py`](running/ui_player_card.py) | Universal Interactive Player Card (Dossier) for the Roboner Audit GUI. |
| [`decision_audit.py`](running/decision_audit.py) | One reader over both decision-record stores. |
| [`narrate.py`](running/narrate.py) | Deterministic plain English for a decision record. |
| [`evidence.py`](running/evidence.py) | The immutable record, flattened into field / value / what it means. |
| [`value_backfill.py`](running/value_backfill.py) | Reconstruct value-table vintages from evidence that predates the archive. |
| [`0_Now.py`](running/0_Now.py) | The slate as it stands: what Roboner would do right now, and on what roster. |
| [`1_Lineups.py`](running/1_Lineups.py) | Weekly Lineup & Start/Sit Decision Audit. |
| [`2_Moves.py`](running/2_Moves.py) | Moves & Waivers Pipeline Audit — Front-to-Back Decision Trace. |
| [`3_Projections.py`](running/3_Projections.py) | Projections & Distributions Hub — Weekly & Rest-of-Season Analysis. |
| [`4_AI_Scout.py`](running/4_AI_Scout.py) | AI & Scout Center — Qualitative LLM Arbitrations & Reporting Intelligence. |
| [`5_Calibration.py`](running/5_Calibration.py) | How the machinery was fitted, and on how much. |
| [`6_Transactions.py`](running/6_Transactions.py) | Transactions — every roster change, one row each, with who asked for it. |
